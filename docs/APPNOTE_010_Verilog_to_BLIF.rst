===========================
Yosys Application Note 010:
Converting Verilog to BLIF
===========================
...
Application Note may deactivate ``TCL``, ``Qt`` and ``MiniSAT`` support
in the ``Makefile`` and may opt against building ``yosys-abc``.

This Application Note is based on GIT Rev. ``e216e0e`` from 2013-11-23
of Yosys :raw-latex:`\cite{yosys}`. The Verilog sources used for the
examples are taken from yosys-bigsim :raw-latex:`\cite{bigsim}`, a
...
processor :raw-latex:`\cite{navre}` is an Open Source AVR clone. It is a
single module (``softusb_navre``) in a single design file
(``softusb_navre.v``). It also is using only features that map nicely to
the BLIF format, for example it only uses synchronous resets.

Converting ``softusb_navre.v`` to ``softusb_navre.blif`` could not be
easier:
...
``-o softusb_navre.blif`` option just adds the command
``write_blif softusb_navre.blif`` to the end of the script. Likewise a
file on the command line – ``softusb_navre.v`` in this case – adds the
command ``read_verilog softusb_navre.v`` to the beginning of the
synthesis script. In both cases the file type is detected from the file
extension.

Finally the option ``-S`` instantiates a built-in default synthesis
script. Instead of using ``-S`` one could also specify the synthesis
commands for the script on the command line using the ``-p`` option,
...
cases it is more convenient to use an actual script file.
...
-  The command ``opt`` is the Yosys’ built-in optimizer. It can perform
...
-  The command ``proc`` converts *processes* (Yosys’ internal
   representation of Verilog ``always``- and ``initial``-blocks) to
   circuits of multiplexers and storage elements (various types of
   flip-flops).

-  The command ``memory`` converts Yosys’ internal representations of
   ...
   option ``-nomap`` is used, in which case the multi-port block
   memories stay in the design and can then be mapped to
   architecture-specific memory primitives using other commands.
...
``hierarchy`` command should be called with the ``-check`` option. This
will then cause synthesis to fail when implicit blackboxes are found in
the design.

The 2nd thing we can improve regarding the ``hierarchy`` command is that
...
finite state machines (FSMs) in the design. The ``fsm`` command finds
...
possible to tell the ``fsm`` command to leave the FSMs in their
...
Our 2nd example is the Amber23 :raw-latex:`\cite{amber}` ARMv2a CPU.
Once again we base our example on the Verilog code that is included in
yosys-bigsim :raw-latex:`\cite{bigsim}`.
...
Listing `[glob_arst] <#glob_arst>`__ are used to define reset values for
...
Listing `[aber23.ys] <#aber23.ys>`__ shows the synthesis script for the
Amber23 core. In line 17 the ``add`` command is used to add a 1-bit wide
global input signal with the name ``globrst``. That means that an input
...
connected throughout the whole design hierarchy.
...
Finally in line 19 the ``techmap`` command is used to replace all
...
Listing `[adff2dff.v] <#adff2dff.v>`__. Note how the
``techmap_celltype`` attribute is used in line 1 to tell the techmap
command which cells to replace in the design, how the ``_TECHMAP_FAIL_``
...
how the ``_TECHMAP_DO_`` wire in line 13 provides a mini
synthesis-script to be used to process this cell.
...
Listings `[aber23.ys] <#aber23.ys>`__ and `[adff2dff.v] <#adff2dff.v>`__
...
successfully executed the program shown in Listing `[sieve] <#sieve>`__
in the test-bench.

For simulation the BLIF file was converted back to Verilog using ABC
:raw-latex:`\cite{ABC}`. So this test includes the successful
...
does not support initialization of arrays with ``$readmemb`` and
``$readmemh``, and has only limited support for tristate logic, to name
...
.. container:: thebibliography
   ...
   | yosys-bigsim, a collection of real-world Verilog designs for
...
.. [1]
   The other half prefers VHDL, a very different but – of course –
   equally powerful language.
