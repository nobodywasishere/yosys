...
This Application Note is based on the Yosys :raw-latex:`\cite{yosys}`
GIT Rev. ``2b90ba1`` from 2013-12-08. The ``README`` file covers how to
install Yosys. The ``show`` command requires a working installation of
GraphViz :raw-latex:`\cite{graphviz}` and :raw-latex:`\cite{xdot}` for
generating the actual circuit diagrams.
...
Sec. `3 <#intro_show>`__ introduces the ``show`` command and explains
the symbols used in the circuit diagrams generated by it.

Sec. `4 <#navigate>`__ introduces additional commands used to navigate
...
Sec. `5 <#poke>`__ introduces commands to evaluate the design and solve
SAT problems within the design.

Sec. `6 <#conclusion>`__ concludes the document and summarizes the key
points.
...
|Output of the three ``show`` commands from
Fig. |\ `[example_src] <#example_src>`__
|image1|\ `[example_src] <#example_src>`__
|image2|\ `[example_src] <#example_src>`__

The ``show`` command generates a circuit diagram for the design in its
...
diagram in a temporary file and launches ``xdot`` to display the
diagram. Subsequent calls to ``show`` re-use the ``xdot`` instance (if
still running).

A simple circuit
----------------

Fig. `[example_src] <#example_src>`__ shows a simple synthesis script
and a Verilog file that demonstrate the usage of ``show`` in a simple
setting. Note that ``show`` is called with the ``-pause`` option, that
halts execution of the Yosys script until the user presses the Enter
key. The ``show -pause`` command also allows the user to enter an
interactive shell to further investigate the circuit before continuing
synthesis.

So this script, when executed, will show the design after each of the
three synthesis commands. The generated circuit diagrams are shown in
Fig. `3 <#example_out>`__.

The first diagram (from top to bottom) shows the design directly after
...
Constants are shown as ellipses with the constant value as label. The
syntax ``<bit_width>’<bits>`` is used for for constants that are not
32-bit wide and/or contain bits that are not 0 or 1 (i.e. ``x`` or
``z``). Ordinary 32-bit constants are written using decimal numbers.

Single-bit signals are shown as thin arrows pointing from the driver to
the load. Signals that are multiple bits wide are shown as think arrows.

Finally *processes* are shown in boxes with round corners. Processes are
Yosys’ internal representation of the decision-trees and synchronization
events modelled in a Verilog ``always``-block. The label reads ``PROC``
followed by a unique identifier in the first line and contains the
source code location of the original ``always``-block in the 2nd line.
Note how the multiplexer from the ``?:``-expression is represented as a
``$mux`` cell but the multiplexer from the ``if``-statement is yet still
hidden within the process.

The ``proc`` command transforms the process from the first diagram into
a multiplexer and a d-type flip-flip, which brings us to the 2nd
diagram.

The Rhombus shape to the right is a dangling wire. (Wire nodes are only
...
two instances of a ``BUF``-node. This are artefacts left behind by the
``proc``-command. It is quite usual to see such artefacts after calling
...
cleaning up after them. The next call to ``clean`` (or ``opt``, which
includes ``clean`` as one of its operations) will clean up this
artefacts. This operation is so common in Yosys scripts that it can
simply be abbreviated with the ``;;`` token, which doubles as separator
...
``clean`` before calling ``show``.

In this script we directly call ``opt`` as next step, which finally
leads us to the 3rd diagram in Fig. `3 <#example_out>`__. Here we see
that the ``opt`` command not only has removed the artifacts left behind
by ``proc``, but also determined correctly that it can remove the first
``$mux`` cell without changing the behavior of the circuit.

.. figure:: APPNOTE_011_Design_Investigation/splice.pdf
   :alt: Output of ``yosys -p ’proc; opt; show’ splice.v``
   :name: splice_dia
...
circuit is a half-adder built from simple CMOS gates.)| |image3|

Break-out boxes for signal vectors
----------------------------------

As has been indicated by the last example, Yosys is can manage signal
...
Fig. `4 <#splice_dia>`__ and `[splice_src] <#splice_src>`__ demonstrates
how such circuits are visualized by the ``show`` command.

The key elements in understanding this circuit diagram are of course the
boxes with round corners and rows labeled
``<MSB_LEFT>:<LSB_LEFT> – <MSB_RIGHT>:<LSB_RIGHT>``. Each of this boxes
has one signal per row on one side and a common signal for all rows on
the other side. The ``<MSB>:<LSB>`` tuples specify which bits of the
signals are broken out and connected. So the top row of the box
connecting the signals ``a`` and ``x`` indicates that the bit 0 (i.e.
the range 0:0) from signal ``a`` is connected to bit 1 (i.e. the range
1:1) of signal ``x``.
...
Finally Fig. `6 <#splitnets_libfile>`__ shows two common pitfalls when
...
awkwardly arranged in a large column. Secondly the two-bit vector ``y``
requires breakout-boxes for its individual bits, resulting in an
unnecessary complex diagram.

For the 2nd diagram Yosys has been given a description of the cell
...
cell library can be passed directly to the ``show`` command using the
``-lib <filename>`` option. Secondly it is possible to load cell
libraries into the design with the ``read_verilog -lib <filename>``
...
In addition to that, the 2nd diagram was generated after
``splitnet -ports`` was run on the design. This command splits all
signal vectors into individual signal bits, which is often desirable
when looking at gate-level circuits. The ``-ports`` option is required
...
launches ``xdot`` to display it. The options ``-format``, ``-viewer``
and ``-prefix`` can be used to change format, viewer and filename
prefix. Note that the ``pdf`` and ``ps`` format are the only formats
that support plotting multiple modules in one run.
...
to find an assignment of colors that looks good.

The command ``help show`` prints a complete listing of all options
...
In addition to *what* to display one also needs to carefully decide
*when* to display it, with respect to the synthesis flow. In general it
...
before calling the ``techmap`` command already fails to verify, it is
better to troubleshoot the coarse-grain version of the circuit before
``techmap`` than the gate-level circuit after ``techmap``.
...
Once the right state within the synthesis flow for debugging the circuit
has been identified, it is recommended to simply add the ``shell``
...
For most cases, the shell will start with the whole design selected
(i.e. when the synthesis script does not already narrow the selection).
The command ``ls`` can now be used to create a list of all modules. The
command ``cd`` can be used to switch to one of the modules (type
``cd ..`` to switch back). Now the ``ls`` command lists the objects
within that module. Fig. `[lscd] <#lscd>`__ demonstrates this using the
design from Fig. `[example_src] <#example_src>`__.

There is a thing to note in Fig. `[lscd] <#lscd>`__: We can see that the
cell names from Fig. `3 <#example_out>`__ are just abbreviations of the
...
``<module_name>/``. For example ``a*/b*`` would refer to all objects
whose names start with ``b`` from all modules whose names start with
``a``.

The ``dump`` command can be used to print all information about an
object. For example ``dump $2`` will print Fig. `[dump2] <#dump2>`__.
...
.. figure:: APPNOTE_011_Design_Investigation/example_03.pdf
   :alt: Output of ``show`` after ``select $2`` or ``select t:$add``
   (see also Fig. `3 <#example_out>`__)
   :name: seladd
...
few exceptions, such as the ``read_`` and ``write_`` commands) operate
...
``show`` command will yield the diagram shown in Fig. `7 <#seladd>`__.
...
``$add``. In this case this is also yields the diagram shown in
Fig. `7 <#seladd>`__.
...
command ``dump t:$add`` will print information on all ``$add`` cells in
the active module. Whenever a command has ``[selection]`` as last
argument in its usage help, this means that it will use the engine
behind the ``select`` command to evaluate additional arguments and use
...
commands ``select -add`` and ``select -del`` can be used to add or
...
.. figure:: APPNOTE_011_Design_Investigation/sumprod_00.pdf
   :alt: Output of ``show a:sumstuff`` on Fig. `[sumprod] <#sumprod>`__
   :name: sumprod_00

   Output of ``show a:sumstuff`` on Fig. `[sumprod] <#sumprod>`__
...
(Try this with the design shown in
Fig. `[foobaraddsub] <#foobaraddsub>`__. Use the ``select -list``
...
the ``%i`` arguments pops the last two elements from the stack,
intersects them, and pushes the result back on the stack. So the
following command will select all $add cells that have the ``foo``
attribute set:
...
The listing in Fig. `[sumprod] <#sumprod>`__ uses the Yosys non-standard
``{... *}`` syntax to set the attribute ``sumstuff`` on all cells
...
shown in Fig. `8 <#sumprod_00>`__. As only the cells themselves are
selected, but not the temporary wire ``$1_Y``, the two adders are shown
...
``select -del clk rst`` and each cell using them will get its own net
label.

In this case however we would like to see the cells connected properly.
This can be achieved using the ``%x`` action, that broadens the
selection, i.e. for each selected wire it selects all cells connected to
the wire and vice versa. So ``show a:sumstuff %x`` yields the diagram
shown in Fig. `9 <#sumprod_01>`__.

.. figure:: APPNOTE_011_Design_Investigation/sumprod_01.pdf
   :alt: Output of ``show a:sumstuff %x`` on
   Fig. `[sumprod] <#sumprod>`__
   :name: sumprod_01

   Output of ``show a:sumstuff %x`` on Fig. `[sumprod] <#sumprod>`__

Selecting logic cones
---------------------

Fig. `9 <#sumprod_01>`__ shows what is called the *input cone* of
``sum``, i.e. all cells and signals that are used to generate the signal
``sum``. The ``%ci`` action can be used to select the input cones of all
object in the top selection in the stack maintained by the ``select``
command.

As the ``%x`` action, this commands broadens the selection by one
...
cells only select wires via input ports.

Fig. `13 <#select_prod>`__ show the sequence of diagrams generated by
...
When selecting many levels of logic, repeating ``%ci`` over and over
...
``%ci3`` is identical to performing the ``%ci`` action three times.

The action ``%ci*`` performs the ``%ci`` action over and over again
until it has no effect anymore.
...
In most cases there are certain cell types and/or ports that should not
be considered for the ``%ci`` action, or we only want to follow certain
cell types and/or ports. This can be achieved using additional patterns
that can be appended to the ``%ci`` action.

Lets consider the design from Fig. `[memdemo_src] <#memdemo_src>`__. It
...
circuit using ``proc; opt; memory; opt`` and change to the ``memdemo``
module with ``cd memdemo``. If we type ``show`` now we see the diagram
shown in Fig. `[memdemo_00] <#memdemo_00>`__.
...
But maybe we are only interested in the tree of multiplexers that select
the output value. In order to get there, we would start by just showing
the output signal and its immediate predecessors:
...
``y`` is connected to the output port ``Q``, that the ``clk`` signal
goes into the ``CLK`` input port of the cell, and that the data comes
from a auto-generated wire into the input ``D`` of the flip-flop cell.

As we are not interested in the clock signal we add an additional
pattern to the ``%ci`` action, that tells it to only follow ports ``Q``
and ``D`` of ``$dff`` cells:
...
To add a pattern we add a colon followed by the pattern to the ``%ci``
action. The pattern it self starts with ``-`` or ``+``, indicating if it
...
list of port names in square brackets.
...
Or we could decide to tell the ``%ci`` action to not follow the ``CLK``
input:
...
Next we would investigate the next logic level by adding another
``%ci2`` to the command:
...
From this we would learn that the next cell is a ``$mux`` cell and we
...
in which the first ``%ci`` jumps over the initial d-type flip-flop and
...
this command is shown in Fig. `14 <#memdemo_01>`__.

Similar to ``%ci`` exists an action ``%co`` to select output cones that
accepts the same syntax for pattern and repetition. The ``%x`` action
mentioned previously also accepts this advanced syntax.

This actions for traversing the circuit graph, combined with the actions
for boolean operations such as intersection (``%i``) and difference
(``%d``) are powerful tools for extracting the relevant portions of the
circuit under investigation.

See ``help select`` for a complete list of actions available in
selections.

Storing and recalling selections
--------------------------------

The current selection can be stored in memory with the command
``select -set <name>``. It can later be recalled using
``select @<name>``. In fact, the ``@<name>`` expression pushes the
stored selection on the stack maintained by the ``select`` command. So
for example
...
will select the intersection between the stored selections ``foo`` and
``bar``.
...
When working with very large modules, it is often not enough to just
...
investigation is encapsulated in a separate module.

Fig. `18 <#submod>`__ shows how the ``submod`` command can be used to
split the circuit from Fig. `[memdemo_src] <#memdemo_src>`__ and
`[memdemo_00] <#memdemo_00>`__ into its components. The ``-name`` option
is used to specify the name of the new module and also the name of the
new cell in the current module.

| |The circuit from
  Fig. |\ `[memdemo_src] <#memdemo_src>`__\ |and|\ `[memdemo_00] <#memdemo_00>`__\ |broken
  up using ``submod``|
...
| |image7|\ `[memdemo_src] <#memdemo_src>`__\ |image8|\ `[memdemo_00] <#memdemo_00>`__\ |image9|

``scramble``

1em

| |image10|\ `[memdemo_src] <#memdemo_src>`__\ |image11|\ `[memdemo_00] <#memdemo_00>`__\ |image12|

``outstage``

1em

| |image13|\ `[memdemo_src] <#memdemo_src>`__\ |image14|\ `[memdemo_00] <#memdemo_00>`__\ |image15|
...
The ``eval`` command can be used to evaluate combinatorial circuits. For
example (see Fig. `18 <#submod>`__ for the circuit diagram of
``selstage``):
...
So the ``-set`` option is used to set input values and the ``-show``
option is used to specify the nets to evaluate. If no ``-show`` option
is specified, all selected output ports are used per default.

If a necessary input value is not given, an error is produced. The
option ``-set-undef`` can be used to instead set all unspecified input
nets to undef (``x``).
...
Note that the ``eval`` command (as well as the ``sat`` command discussed
...
``eval -table`` command. For larger circuits however, Yosys provides the
``sat`` command that uses a SAT :raw-latex:`\cite{CircuitSAT}` solver
:raw-latex:`\cite{MiniSAT}` to solve this kind of problems.
...
the corresponding input values. For Example:
...
the ``-set`` option. In the above example we used ``-set s1 s2`` to
constraint ``s1`` and ``s2`` to be equal. When more complex constraints
...
port, which then can be forced to a value using the ``-set`` option.
(Such a circuit that contains the circuit under test plus additional
constraint checking circuitry is called a *miter* circuit.)

Fig. `[primetest] <#primetest>`__ shows a miter circuit that is supposed
to be used as a prime number test. If ``ok`` is 1 for all input values
``a`` and ``b`` for a given ``p``, then ``p`` is prime, or at least that
is the idea.

The Yosys shell session shown in Fig. `[primesat] <#primesat>`__
...
test in 32 bits, for example by replacing ``p != a*b`` in the miter with
``p != {16’d0,a}b``, or by using a temporary variable for the 32 bit
product ``a*b``. But as 31 fits well into 8 bits (and as the purpose of
...
done in the second command in Fig. `[primesat] <#primesat>`__ (line 31).

The ``-prove`` option used in this example works similar to ``-set``,
...
It might be worth noting, that SAT solvers are not particularly
...
is perfectly capable of solving it.
...
consider the entire ``memdemo`` module from
Fig. `[memdemo_src] <#memdemo_src>`__ and suppose we want to know which
sequence of input values for ``d`` will cause the output ``y`` to
produce the sequence 1, 2, 3 from any initial state.
Fig. `[memdemo_sat] <#memdemo_sat>`__ show the solution to this
question, as produced by the following command:
...
The ``-seq 6`` option instructs the ``sat`` command to solve a
...
circuit in a state from which the sequence 1, 2, 3 can be produced.)

The ``-set-init-undef`` option tells the ``sat`` command to initialize
all registers to the undef (``x``) state. The way the ``x`` state is
...
which inputs bits are relevant to the solution.

Finally the three ``-set-at`` options add constraints for the ``y``
signal to play the 1, 2, 3 sequence, starting with time step 4.

It is not surprising that the solution sets ``d = 0`` in the first step,
as this is the only way of setting the ``s1`` and ``s2`` registers to a
...
There is much more to write about the ``sat`` command. For example,
there is a set of options that can be used to performs sequential proofs
using temporal induction :raw-latex:`\cite{tip}`. The command
``help sat`` can be used to print a list of all options with short
...
values (``eval``) or by evaluation which input values and initial
conditions can result in a certain behavior at the outputs (``sat``).
...
.. container:: thebibliography
   ...
   xdot.py - an interactive viewer for graphs written in Graphviz’s dot
   language. https://github.com/jrfonseca/xdot.py

   *Circuit satisfiability problem* on Wikipedia
   http://en.wikipedia.org/wiki/Circuit_satisfiability

   MiniSat: a minimalistic open-source SAT solver. http://minisat.se/

   Niklas Een and Niklas Sörensson (2003). Temporal Induction by
   Incremental SAT Solving.
   http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.4.8161

.. |Output of the three ``show`` commands from Fig. | image:: APPNOTE_011_Design_Investigation/example_00.pdf
   :name: example_out
.. |image1| image:: APPNOTE_011_Design_Investigation/example_01.pdf
   :name: example_out
.. |image2| image:: APPNOTE_011_Design_Investigation/example_02.pdf
   :name: example_out
.. |Effects of ``splitnets`` command and of providing a cell library. (The circuit is a half-adder built from simple CMOS gates.)| image:: APPNOTE_011_Design_Investigation/cmos_00.pdf
   :name: splitnets_libfile
.. |image3| image:: APPNOTE_011_Design_Investigation/cmos_01.pdf
   :name: splitnets_libfile
.. |Objects selected by ``select prod %ci...``| image:: APPNOTE_011_Design_Investigation/sumprod_02.pdf
   :name: select_prod
   :width: 4cm
.. |image4| image:: APPNOTE_011_Design_Investigation/sumprod_03.pdf
   :name: select_prod
.. |image5| image:: APPNOTE_011_Design_Investigation/sumprod_04.pdf
   :name: select_prod
.. |image6| image:: APPNOTE_011_Design_Investigation/sumprod_05.pdf
   :name: select_prod
.. |image| image:: APPNOTE_011_Design_Investigation/memdemo_00.pdf
.. |Output of ``show y %ci2:+$dff[Q,D] %ci*:-$mux[S]:-$dff``| image:: APPNOTE_011_Design_Investigation/memdemo_01.pdf
   :name: memdemo_01
.. |The circuit from Fig. | image:: APPNOTE_011_Design_Investigation/submod_00.pdf
   :name: submod
.. |and| image:: APPNOTE_011_Design_Investigation/submod_00.pdf
   :name: submod
.. |broken up using ``submod``| image:: APPNOTE_011_Design_Investigation/submod_00.pdf
   :name: submod
.. |image7| image:: APPNOTE_011_Design_Investigation/submod_01.pdf
   :name: submod
.. |image8| image:: APPNOTE_011_Design_Investigation/submod_01.pdf
   :name: submod
.. |image9| image:: APPNOTE_011_Design_Investigation/submod_01.pdf
   :name: submod
.. |image10| image:: APPNOTE_011_Design_Investigation/submod_02.pdf
   :name: submod
.. |image11| image:: APPNOTE_011_Design_Investigation/submod_02.pdf
   :name: submod
.. |image12| image:: APPNOTE_011_Design_Investigation/submod_02.pdf
   :name: submod
.. |image13| image:: APPNOTE_011_Design_Investigation/submod_03.pdf
   :name: submod
.. |image14| image:: APPNOTE_011_Design_Investigation/submod_03.pdf
   :name: submod
.. |image15| image:: APPNOTE_011_Design_Investigation/submod_03.pdf
   :name: submod
